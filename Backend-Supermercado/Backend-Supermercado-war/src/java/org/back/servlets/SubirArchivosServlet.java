	                    ...
	                    if (!item.isFormField()) {
	                        ...
	                        File path = new File(directorio);
	                            ...
	                            boolean status = path.mkdirs();
                                    ...
                                    // Máximo tamaño permitido 1 MB
                                    if(item.getSize() < (1024*1000)){
                                        System.out.println(uploadedFile.getAbsolutePath()+" tamaño: "+uploadedFile.length());
                                        rutaArchivo = BackConstantes.RUTA_ARCHIVOS_UPLOAD+uploadedFile.getName();
                                        item.write(uploadedFile);
                                        session.setAttribute("archivo", uploadedFile);
                                        archivoGuardado = true;
                                    } else {
                                        mensaje_error = "El tamaño del archivo excede el máximo permitido (1MB).";
                                    }
                                } else {
                                     mensaje_error = "El tipo del archivo no es el permitido (Sólo .png, .gif, .jpeg, .jpg).";
                                }
	                    }
	                }
